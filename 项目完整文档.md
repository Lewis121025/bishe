...
> 残差即超额薪酬（Overpay），经 1%/99% Winsorize 缩尾；lnSale 非正值置缺失（不再 clip）
...
│   ├── regression_tables.txt     # 所有数值的唯一权威来源
...
## 10. 当前实证结果（2026-03-21 修正版）

> **本次修正**：修正 Zone 区域分类（重庆移至西部）、lnSale 非正值处理、Overpay 残差 Winsorize、期望薪酬模型加入年份固定效应，并重新运行全部回归。

### 10.1 主回归（统一样本 N = 44,861）
...
结论：财政补贴与高管超额薪酬显著正相关（H1 成立）。

### 10.2 中介效应

- Power 系数 0.0066，t = 1.37，不显著
- Sobel 检验不显著，间接效应占比约 1.16%

结论：全样本下管理层权力中介效应不成立（H2 不支持）。
...
- Power 在央企（p<0.1）和地方国企（p<0.05）均显著，支持中介机制的制度依赖性。

### 10.4 稳健性

| 检验 | 系数 | t 值 |
|:---|:---:|:---:|
| (1) 替换因变量 lnCEOpay | 0.1319*** | 32.94 |
| (2) 缩小期间 2010-2020 | 0.0536*** | 12.48 |
| (3) 剔除极端补助值 | 0.0577*** | 12.91 |
| (4) 仅制造业 | 0.0709*** | 14.83 |
| (5) ln(1+补助) | 0.0509*** | 14.77 |
| (6) 滞后一期补助 | 0.0508*** | 13.92 |

6 组稳健性检验中，补贴系数均保持正向强显著，结论高度稳健。
