...
def main() -> int:
    ...
    sample_chain = (
        ...
        f"在主回归中加入滞后一期补助变量后，模型2样本为{fmt_int(sample_map['模型2主回归样本']['observations'])}条；"
    ...
    expect(
        thesis,
        f"整体 KMO 约为{fmt_plain(diagnostics['power_fa']['kmo_overall'], 3)}，单因子方差解释率约为{float(diagnostics['power_fa']['variance_explained_ratio']) * 100:.2f}%",
        "FA 统计支撑说明",
        failures,
    )
    expect(
    ...
    desc_rows = [
        ...
        f"| 管理层权力（Power，FA） | {fmt_int(desc_map['管理层权力(Power, FA口径)']['N'])} | {fmt_plain(desc_map['管理层权力(Power, FA口径)']['均值'])} | {fmt_plain(desc_map['管理层权力(Power, FA口径)']['中位数'])} | {fmt_plain(desc_map['管理层权力(Power, FA口径)']['标准差'])} | {fmt_fixed(desc_map['管理层权力(Power, FA口径)']['最小值'])} | {fmt_plain(desc_map['管理层权力(Power, FA口径)']['最大值'])} |",
    ]
    for idx, row in enumerate(desc_rows, start=1):
        expect(thesis, row, f"描述性统计行{idx}", failures)

    expect(thesis, "| lnSubsidy | 1.0900 |", "VIF 行 lnSubsidy", failures)
    expect(thesis, "| Lever | 1.6732 |", "VIF 行 Lever", failures)
    expect(thesis, "均值约为1.2472", "VIF 均值说明", failures)
    ...
    for var, coef in stage1_expect.items():
    ...
    iv_first = causal_map[("IV-第一阶段", "FE-2SLS_FS")]
    iv_second = causal_map[("IV-第二阶段", "FE-2SLS_SS")]
    expect(thesis, f"| 第一阶段 | lnSubsidy_l1 | {fmt_plain(iv_first['coef'])}*** | Partial F = {fmt_plain(diagnostics['iv']['partial_f'], 2)} | {fmt_int(iv_first['n'])} |", "IV 第一阶段行", failures)
    expect(thesis, f"| 第二阶段 | Overpay | {fmt_fixed(iv_second['coef'])} | t = {fmt_fixed(diagnostics['iv']['second_stage_t'], 4, unicode_minus=False)} | {fmt_int(iv_second['n'])} |", "IV 第二阶段行", failures)
    expect(thesis, f"Partial $R^2$ 为{fmt_plain(diagnostics['iv']['partial_r2'])}", "IV Partial R² 说明", failures)
    ...
    expect(thesis, f"| 模型3 总效应 c：lnSubsidy_l1 → Overpay | {fmt_plain(med['coef_c'])} | {fmt_plain(med_c['se'])} | — |", "中介总效应行", failures)
    expect(thesis, f"| 模型4 路径 a：lnSubsidy_l1 → Power（FA） | {fmt_plain(med['coef_a'])} | {fmt_plain(med_a['se'])} | — |", "中介路径a行", failures)
    expect(thesis, f"| 模型5 路径 b：Power（FA） → Overpay | {fmt_plain(med['coef_b'])}*** | {fmt_plain(med_b['se'])} | — |", "中介路径b行", failures)
    expect(thesis, f"| 模型5 直接效应 c'：lnSubsidy_l1 → Overpay | {fmt_plain(med['coef_c_prime'])} | {fmt_plain(med_c_prime['se'])} | — |", "中介直接效应行", failures)
    ...
    expect(thesis, "| lnSubsidy_l1 | −0.0000（−0.01） | −0.0002（−0.12） |", "产权异质性核心行", failures)
    ...
    expect(thesis, "| lnSubsidy_l1 | −0.0005（−0.31） | 0.0008（0.78） |", "行业异质性核心行", failures)
    expect(thesis, f"| N | {fmt_int(mech_map[('行业管制', '管制行业')]['sample_size'])} | {fmt_int(mech_map[('行业管制', '非管制行业')]['sample_size'])} |", "行业异质性样本量", failures)
    expect(thesis, "| lnSubsidy_l1 | −0.0006（−0.32） | −0.0013（−0.91） |", "央地异质性核心行", failures)
    expect(thesis, "央地可识别比例为74.50%", "央地可识别比例", failures)
    ...
    expect(thesis, "`Roa`、`Mgshder` 和 `lnSubsidy` 位于最重要的几类特征之中", "SHAP 排序描述", failures)
    ...
    expect(thesis, "（$\\beta = 0.0005$，$p = 0.603$）", "摘要主回归值", failures)
    expect(thesis, "路径$a$为正但未达显著水平（$p = 0.119$）", "摘要路径a", failures)
    expect(thesis, "间接效应约为0.000055，Bootstrap 95%置信区间为[-0.000004, 0.000132]", "摘要间接效应", failures)
    expect(thesis, "XGBoost 回归的测试集 $R^2$ 仅为0.1006，分类模型的 ROC-AUC 约为0.619", "结论机器学习值", failures)

    stale_values = [
        "45,651", "36,340", "9,311", "22,457", "23,194", "49.19%",
        "74.88%", "0.1037", "0.6228", "0.138", "0.000008", "1.52", "1.22",
