...
本文以2003—2024年沪深A股非金融上市公司为样本，考察财政补贴与高管超额薪酬之间的关系。结果显示，全样本固定效应下补贴系数为正但不显著，多种工具变量、安慰剂、事件研究和 Heckman 检验给出的识别结果较为敏感，现有证据尚未形成一致支持。管理层权力中介效应未获稳健支持；机器学习部分主要用于补充说明变量信息含量、局部结构和模型比较，不替代第四章的计量结论。总体而言，财政补贴与高管超额薪酬之间存在一定关联，但不足以支持无条件的强因果结论。
...
Using non-financial A-share listed firms from 2003 to 2024, this thesis examines the relationship between fiscal subsidies and executive excess compensation. The fixed-effects results show a positive but insignificant coefficient for fiscal subsidies, and additional instrumental-variable tests, placebo tests, event-study evidence, and Heckman correction results remain sensitive across specifications. The mediating effect of managerial power is not robustly supported. Machine learning methods are used only to supplement the analysis of variable information content, local structure, and model comparison, rather than to replace the econometric evidence. Overall, fiscal subsidies are associated with executive excess compensation to some extent, but the evidence does not support an unconditional strong causal conclusion.
